  ...
  listenForPaymentEvent = () => {
    ...
    this.itemManager.events.SupplyChainStep().on("data", async e => {
      if (e.returnValues._step == 1) {
        let itemObj = await self.itemManager.methods.items(e.returnValues._itemIndex).call();
        console.log(itemObj);
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        <h2>Items</h2>
        <h2>Add Items</h2>
        Cost in Wei: <input type="text" name="cost" value={this.state.value} onChange={this.handleInputChange} />
